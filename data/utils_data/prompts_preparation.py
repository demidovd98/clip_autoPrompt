...
RAND = 1697449792 # 100_cifar: 1684427807, 10_cifar: 1684427942 ; 1000_100c_strict10_new: 1683237191, 100_100c_strict10_new: 1683237344, # 1000_100c_strict: 1682947137, # 100_100c_strict_new: 1683038132, # 100_100c_strict: 1682986352, # 100_100c: 1682891271, # 10_100c_strict: 1683038085 # 10_100c: 1682986245 # 100_12c: 1678543337 # 1000_12c: 1678705786
...
if DATASET == 'cifar10':
    plurals = ['s', 's', 's']
    classes_temp = classes_cifar
else:
    plurals = ['s', 'es', 'ies']

    if DIFFICULTY == 'easy':
        classes_temp = classes_easy
    elif DIFFICULTY == 'mid':
        classes_temp = classes_mid_100 #classes_mid
    elif DIFFICULTY == 'full':
        classes_temp = classes_full
    else:
...
def prepare_prompts(dataset, folder, classes, difficulty, numb_of_sent, division, plurals, strict=True, strict_symb=10, options=True, one_class=False, old=False, prepare_only=False, file_name_tsv=None, rand=None):
    ...
    OLD = old # False with b'text' for 12 classes
    STRICT = strict #True
    ...
    if not os.path.exists(folder_prompts):
        os.makedirs(folder_prompts)


    skipped_sent = 0
    skipped_sent_strict = 0
    ...
    for hyponym in tqdm(classes_temp):
        ...
        with open(path, "r") as input:
            with open(folder_prompts + \
                    #"/prompts_" + str(DIFFICULTY) + "_" + "sent" + str(NUM_OF_SENTENCES) + "_" + str(DIVISION) + "_rand" + str(epoch_time) + ".tsv", "w") as output:
    ...
    print("[INFO] Non-strict class skipped: ", skipped_sent_strict, flush=True)
    print("[INFO] Foreign class occurencies skipped: ", skipped_sent, flush=True)
