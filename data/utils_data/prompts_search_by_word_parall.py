...
OPTIONS = True # True
#PLURALS = True
PRINT_DETAILS = False # no details printed
...
classes_full = list(classes_full_dict.values())
#print(classes_full)
...
if DATASET == 'cifar10':
    ...
    if DIFFICULTY == 'easy':
        classes = classes_easy
    elif DIFFICULTY == 'mid':
        classes = classes_mid_100 #classes_mid
    elif DIFFICULTY == 'full':
        classes = classes_full
    else:
        print("Chose difficulty level")

#classes_id = [ ]
'''


def search_prompts(dataset, folder, classes, difficulty, numb_of_sent, plurals, strict=True, strict_symb=10, options=True):
    ...
    folder = folder #"/l/users/20020067/Activities/CLIP_prompts/clip_autoPrompt/data/prompts_our/" + str(DATASET) + "_cls" + str(len(classes)) + "_" + str(epoch_time)
    if not os.path.exists(folder):
        os.makedirs(folder)

    # # for hyponym in tqdm(df.hyponym):
    # #for hyponym in tqdm(df_test.hyponym):
    # for hyponym in tqdm(classes):
    #     print(hyponym)

    #with Pool() as pool:
    with Pool(processes=30) as pool:
        #pool.map(hyponym_parall, classes)

        pool.map(partial(hyponym_parall, folder=folder, classes=classes, difficulty=difficulty, 
                         numb_of_sent=numb_of_sent, plurals=plurals, rand=epoch_time, 
                         strict=strict, strict_symb=strict_symb, options=options), 
                         classes) # list to iterate through

    print(f"[INFO] Total search time: {monotonic() - start_time} seconds")





#def hyponym_parall(hyponym):
def hyponym_parall(hyponym, folder, classes, difficulty, numb_of_sent, plurals, rand, strict=True, strict_symb=10, options=True):

    start_time_local = monotonic()
    ...
    #with open("/home/demidova/docs/projects/tax/tax_t5/dataset_test.tsv", "w") as output: 
    with open(folder + "/sentences_" + str(DIFFICULTY) + "_sent" + str(NUM_OF_SENTENCES) \
                    + "_hyponyM" + str(hyponym) + "_rand" + str(epoch_time) + ".tsv", "w") as output:


        print("[INFO] Start search for hyponym:", hyponym, flush=True)
        ...
        # if check%100 == 0:
        #     if check > 0: print(f'{check} words found')
        ...
        for i in range(NUM_OF_FILES): 
            ...
            #with open("/home/demidova/docs/projects/tax/tax_t5/UMBC_webbase_all.tar.gz/UMBC_webbase_all/"+names[ind_names]+str(index)+".possf2") as input: 
            with open("/l/users/20020067/Datasets/UMBC_corpus/UMBC_webbase_all/" + names[ind_names] + str(index) + ".possf2", 'r', encoding='utf-8') as input: 
            ...
            for line in lines:
                ...
                if not line.strip():
                    ...
                    if OPTIONS:
                        hyponym_list = hyponym.split(", ")
                    else:
                        hyponym_list = [hyponym]

                    #if hyponym in line:
                    for hyponym_item in hyponym_list:
                        #print(hyponym_item)
                        hyponym_item = hyponym_item.lower()

                        if hyponym_item in sentence:
                            if STRICT:
                                if len(hyponym_item.split(" ")) <= 1:
                                    if len(hyponym_item) <= STRICT_SYMB:
                                        words = sentence.split(' ')        

                                        word_match = False
                                        for word in words:
                                            #if (hyponym_item == word) or ((hyponym_item +'s') == word) or ((hyponym_item +'es') == word) or ((hyponym_item[:-1] +'ies') == word):
                                            if (hyponym_item == word) or ((hyponym_item + plurals[0]) == word) or ((hyponym_item + plurals[1]) == word) or ((hyponym_item[:-1] + plurals[2]) == word):
                                                word_match = True
                            ...
                            if PRINT_DETAILS:
                                if count%100 == 0:
                                    if count > 0: print(f'[INFO] {count} words found for word:', hyponym, flush=True)

                            if count >= NUM_OF_SENTENCES:
                                print("[INFO] Max number of senteces found for word: ", hyponym, f", after {monotonic() - start_time_local} seconds", flush=True)

                            if PRINT_DETAILS: print(count)                            
                            if PRINT_DETAILS: print(hyponym_item)
                            if PRINT_DETAILS: print(sentence)
    ...
    if count != NUM_OF_SENTENCES: print(f"[WARNING] For word {hyponym}, total number of senteces found: {count}", flush=True)




# from prompts_preparation import prepare_prompts

# #DIVISION = '2words' # 'Sent', '7words', '6words', '5words', '4words', '3words', '2words', '1words'

# prepare_prompts(dataset=DATASET, classes=classes, difficulty=DIFFICULTY, numb_of_sent=NUM_OF_SENTENCES, rand=epoch_time, division, strict=STRICT, strict_symb=STRICT_SYMB, options=OPTIONS, one_class=False, old=False)
