...
NUM_OF_SENTENCES = 100 # 1, 10, 100, 1000
...
#def search_prompts(dataset, classes, difficulty, numb_of_sent, plurals, strict=True, strict_symb=10, options=True):
...
folder = "/l/users/20020067/Activities/CLIP_prompts/clip_autoPrompt/data/prompts_our/" + str(DATASET) + "_" + str(epoch_time)
...
#def hyponym_parall(hyponym, folder, classes, difficulty, numb_of_sent, plurals, rand, strict=True, strict_symb=10, options=True):
    ...
    with open(folder + "/sentences_" + str(DIFFICULTY) + "_sent" + str(NUM_OF_SENTENCES) \
        ...
        print("Start search for hyponym:", hyponym, flush=True)
                ...
                if not line.strip():
                    ...
                    for hyponym_item in hyponym_list:
                        ...
                        if hyponym_item in sentence:
                            ...
                            if PRINT_DETAILS:
                                if count%100 == 0:
                                    if count > 0: print(f'{count} words found for word:', hyponym, flush=True)

                            if count >= NUM_OF_SENTENCES:
                                print("Max number of senteces found for word: ", hyponym, flush=True)
    ...
    #print("Number of words done:", words_done, ", out of ", len(classes))
